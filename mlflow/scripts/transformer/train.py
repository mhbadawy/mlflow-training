...
# Detect number of GPUs (SLURM will set WORLD_SIZE)
...
# Configure MLflow — must happen before Trainer is constructed so that the
# MLflowCallback (activated by report_to="mlflow") picks up the right experiment.
...
with mlflow.start_run(run_name=args.mlflow_run_group, log_system_metrics=True) as run:
    # Trainer is built inside the active run so its MLflowCallback logs into
    # this run automatically (loss, eval metrics, hyperparams, etc.).
    trainer = Trainer(
        model=model,
        args=TRAINING_ARGS,
